...
async def remove_current_channel(client, message: Message):
    ...
    try:
        if await db.is_channel_in_group(channel_id, group):
...
async def list_channels(client, message: Message):
    ...
    for channel_data in channels:
        # Read straight from DB — names are kept fresh by the background
        # sync_channel_names() job that runs at bot startup, so no live
        # get_chat() call is needed here anymore.
        channel_id = channel_data.get("channel_id")
        channel_name = channel_data.get("name") or "Unknown Channel"

        line = f"**{counter}. {channel_name}**\n🆔 : `{channel_id}`\n"
