...
class Database:
    ...
    # ============ Invite Link System ============ #
    async def save_invite_link(self, channel_id_str, data: Dict):
        """Store/replace the current invite link for a channel"""
        try:
            ...
            await self.log_error(f"Error saving invite link: {e}")
            return False

    async def get_invite_links_by_group(self, group="0"):
        """Get all stored invite links for a group"""
        try:
            return [doc async for doc in self.invite_links.find({"group": group})]
        except Exception as e:
            await self.log_error(f"Error getting invite links: {e}")
            return []

    async def delete_invite_link(self, channel_id_str):
        try:
            await self.invite_links.delete_one({"_id": channel_id_str})
            return True
        except Exception as e:
            await self.log_error(f"Error deleting invite link: {e}")
            return False
    ...
    async def get_pending_deletions(self):
        try:
            return await self.posts.find({
                "delete_after": {"$exists": True, "$ne": None}
            }).to_list(None)
        except Exception as e:
